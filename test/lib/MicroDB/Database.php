...
require_once $_SERVER['DOCUMENT_ROOT'] . "/test/lib/yaml.php";
use Yaml;
...
class Database
{
    ...
    public function save($id, $data)
    {
        ...
        return $this->synchronized($id, function () use ($self, $event) {
            ...
            $self->put($this->path . $this->generateSubTree($event->id) . ".yml", $yaml->dump($event->data));
    ...
    public function load($id, $key = null)
    {
        ...
        $event->data = $yaml->loadString($this->get($this->path . $this->generateSubTree($event->id) . ".yml"), true);
    ...
    {
        return is_file($this->path . $id . ".yml");
    }
