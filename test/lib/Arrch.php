...
class Arrch
{
	...
	public static function custom_array_merge(&$array1, &$array2, $key, $foreign) {
		$result = Array();
		foreach ($array1 as $key_1 => &$value_1) {
			// if($value['name'])
			foreach ($array2 as $key_1 => $value_2) {
				if(isset($value_2[$foreign]) && isset($value_1[$key]) && $value_1[$key] ==  $value_2[$foreign]) {
					$result[] = array_merge($value_1,$value_2);
				} else if(!isset($value_1[$key])) {
					$result[] = $value_1;
					break;
				}
			}
