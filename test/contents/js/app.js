...
Vue.component('account-component',{
	template :` 
			...
			<ul class="custom-menu">
				...
				<li @click="logout"><a href="#"><i class="fa fa-unlock-alt"></i> Logout</a></li>
...
Vue.component('search-component', {
	template: [
		'<form>',
			'<input class="input search-input" type="text" :placeholder="dict.EnterKeyword" v-model="searchTerm" @keyup="updatesearch($event.target.value)">',
			'<select class="input search-categories">',
				'<option v-for="cat in categories" :value="cat.code">{{dict[cat.label]}}</option>',
			'</select>',
			'<button @click="updatesearch(searchTerm)" class="search-btn"><i class="fa fa-search"></i></button>',
		'</form>'
	].toString().replace(/\,/g,""),
	data: function(){
		return {
...
var ecommerce = new Vue({
  ...
  methods: {
	  setSearch : function(val){
		  this.search = val;
		  eventHub.$emit('reload-products', this.search);
	  },
	  logout : function(){
		  this.user = false;
	  },
